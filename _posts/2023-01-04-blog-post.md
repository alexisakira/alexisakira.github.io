...
For managing bibliographies for my papers, I have been using the `natbib` package with `bibtex` for a long time.
...
On a separate note, I have been using the `biblatex` package (without understanding it) to create my [CV](/files/alexisakira_toda_CV.pdf). `biblatex` is convenient because the user can easily customize the reference list, for instance listing research articles, book chapters, conference proceedings, working papers, etc. in different sections (with potentially the same or different numbering and/or reverse chronologically).
