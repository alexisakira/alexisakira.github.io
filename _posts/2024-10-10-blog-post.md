...
As I prepare for my first teaching assignments at Emory, I have started by creating a syllabus template. My department provided a Word template, but it wasn’t exactly to my taste, so I wrote a \\(\LaTeX\\) template [here](/files/syllabus_template.zip). The version titled ``syllabus_template.tex`` follows the basic structure of the Emory economics template, and the version titled ``syllabus_template_toda.tex`` adds some of my information that I use across all my courses. A few comments:
